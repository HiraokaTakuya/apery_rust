...
impl Thread {
    ...
    fn search<IsPv: Bool>(&mut self, stack: &mut [Stack], alpha: Value, beta: Value, mut depth: Depth, cut_node: bool) -> Value {
        ...
        if get_stack(stack, 0).in_check {
                ...
                && (get_stack(stack, 0).ply >= self.null_move_pruning_min_ply || us != self.null_move_pruning_color)
            {
                ...
                let r = Depth((982 + 85 * depth.0) / 256 + std::cmp::min((eval.0 - beta.0) / 192, 3));
        ...
        while let Some(m) = mp.next_move(&self.position, move_count_pruning) {
            ...
            if !root_node && best_value > Value::MATED_IN_MAX_PLY {
                ...
                if !is_capture_or_pawn_promotion && !gives_check {
                    ...
                    if lmr_depth < Depth(7)
                        ...
                        && get_stack(stack, 0).static_eval.0 + 283 + 170 * lmr_depth.0 <= alpha.0
                    ...
                    if !self
                        .position
                        .see_ge(m, Value(-(29 - std::cmp::min(lmr_depth.0, 18)) * lmr_depth.0 * lmr_depth.0))
                    {
                        continue;
                    }
                } else {
            ...
            let (do_full_depth_search, did_lmr) = if depth.0 >= 3
                ...
                && (!is_capture_or_pawn_promotion
                    ...
                    || cut_node
                    || self.tt_hit_average < 427 * TT_HIT_AVERAGE_RESOLUTION * TT_HIT_AVERAGE_WINDOW / 1024)
            {
                ...
                if (root_node || !pv_node) && depth > Depth(10) && self.best_move_changes.load(Ordering::Relaxed) <= 2 {
                    r += Depth::ONE_PLY;
                }
                ...
                if !is_capture_or_pawn_promotion {
                    ...
                    if get_stack(stack, 0).stat_score >= -106 && get_stack(stack, -1).stat_score < -104 {
                        r -= Depth::ONE_PLY;
                    } else if get_stack(stack, -1).stat_score >= -119 && get_stack(stack, 0).stat_score < -140 {
                        r += Depth::ONE_PLY;
                    }
    ...
    fn qsearch<IsPv: Bool>(&mut self, stack: &mut [Stack], alpha: Value, beta: Value, depth: Depth) -> Value {
        ...
        if get_stack(stack, 0).in_check {
            ...
            futility_base = best_value + Value(145);
        }
